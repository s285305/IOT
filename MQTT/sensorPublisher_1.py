...
class PolePublisher:
    ...
    def __init__(self, coordinates):
        ...
        # ID unico
        self.client_id = f"PolePublisher_{self.lat}_{self.long}"
    ...
    def publish_unregister(self):
        msg = {"id": self.client_id, "message": "unregister"}
        # publish on the same topic the gateway already listens to
        self.client.publish(self.this_topic, json.dumps(msg), qos=1)
    ...
    def on_cmd_message(self, client, userdata, msg):
        ...
        if cmd == "deactivate":
            ...
            self.publish_unregister()     #invoia messaggio di unregister e delete pole
            time.sleep(0.2)               # small delay to flush publish
    ...
    def finalize(self):
        ...
        time.sleep(0.2)   # per dare tempo al publish (QoS1) di uscire
        self.client.loop_stop()
        self.client.disconnect()
