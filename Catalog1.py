...
# mettere i get, valuta i parametri
        ...
        if os.path.exists(self.filename):
            ...
            self.data = {
                ...
                "db_info": { "influx_host": "http://localhost:8181","token": "apiv3_qkhm2kiMdFeqH7127QV_5-6dak5IwIrzr7Fy85eEnYpyXs2o46NdAvqqYm8BLUOWKxAO83-3UuoB9A9ikLpXVQ",
                            "db": "pole_measurements","writer_port": 8090},
                "dashboard":"",
                "threshold": 20,
                "regions": {  "Piemonte": {
                "minLat": 44.1,
...
class PoleAPI:
    ...
    def POST(self, *uri, **params):
        ...
        # servono coordinate per replacement
        ...
        # default: se non esiste active, consideriamo attivo
        for p in current_poles:
            if "active" not in p:
                p["active"] = True

        # 1) Se esiste già un palo con lo stesso ID: aggiorna quel palo (no replacement)
        for k, existing in enumerate(current_poles):
            if existing.get("id") == new_id:
                #impedisco update di pali innattivi 
                if existing.get("active") is False:
                    raise cherrypy.HTTPError(409, "Questo pole_id è inattivo (storico). Usa un nuovo id.")

                pole.setdefault("active", existing.get("active", True))
                current_poles[k] = pole
                self.catalog.data["gateways"][i]["smart_poles"] = current_poles
                self.catalog.data["gateways"][i]["last_update"] = time.time()
                self.catalog.save()
                return {"status": "pole_updated", "gateway_id": gateway_id, "id": new_id}

        # 2) Replacement: cerca un palo ATTIVO con stesse coordinate
        EPS = 1e-5  # ~1 metro circa
        replaced = None
        replaced_idx = None

        for k, existing in enumerate(current_poles):
            if existing.get("active", True) and self._same_location(existing, new_lat, new_lon, eps=EPS):
                replaced = existing
                replaced_idx = k
                break

        pole["active"] = True
        pole.setdefault("installed_at", time.time())

        # se sostituisci un palo: old -> inactive, new -> active (id diversi)
        if replaced is not None:
            if replaced.get("id") == new_id:
                raise cherrypy.HTTPError(400, "Il nuovo palo deve avere un id diverso dal vecchio")  # requisito utente

            replaced["active"] = False
            replaced["replaced_by"] = new_id
            replaced["replaced_at"] = time.time()
            current_poles[replaced_idx] = replaced

        # aggiungi sempre il nuovo palo (anche se stessa posizione)
        ...
        return {
            ...
            "id": new_id,
            "replaced_old_pole_id": replaced.get("id") if replaced is not None else None
        }
...
class RootAPI:
    ...
    def __init__(self, catalog: SmartCityCatalog):
        ...
        self.backend = BackEndAPI(catalog)  # /backend
    ...
    def GET(self, *uri, **params):
        ...
        if uri[0]=='BackEnd':
            return self.catalog.data.get('BackEnd', {})
